...
if grep -A2 "eth1" configuration1.txt | grep -A2 "subnet 10.0.0.0/24" | grep -q "10.0.0.1"; then
...
if grep -A2 "eth2" configuration1.txt | grep -A2 "subnet 10.0.1.0/24" | grep -q "10.0.1.1"; then
...
if grep -A2 "eth1" configuration2.txt | grep -A2 "subnet 192.168.0.0/24" | grep -q "192.168.0.1"; then
...
if grep -A2 "eth2" configuration2.txt | grep -A2 "subnet 192.168.2.0/24" | grep -q "192.168.2.1"; then
...
if grep -A1 "64 bytes from" Firewall1_eth3.txt | grep -q "time="; then
...
if grep -A1 "64 bytes from" Firewall2_ping.txt | grep -q "time="; then
